...
// ============================================================================
// CONFIGURABLE GRID SIZE - MUST MATCH SERVER!
// ============================================================================
// Change these values to use different grid sizes:
//   4x4 grid:  GRID_SIZE=4,  BOX_ROWS=2, BOX_COLS=2, MAX_NUM=4
//   6x6 grid:  GRID_SIZE=6,  BOX_ROWS=2, BOX_COLS=3, MAX_NUM=6
//   9x9 grid:  GRID_SIZE=9,  BOX_ROWS=3, BOX_COLS=3, MAX_NUM=9  (standard)
//   12x12 grid: GRID_SIZE=12, BOX_ROWS=3, BOX_COLS=4, MAX_NUM=12
//   16x16 grid: GRID_SIZE=16, BOX_ROWS=4, BOX_COLS=4, MAX_NUM=16
// ============================================================================
...
void print_game_rules(void) {
    ...
    printf("|                                                           |\n");
    printf("|  HOW TO PLAY:                                             |\n");
    printf("|  * On your turn, place a number (1-%d) in an empty cell   %s|\n", 
           MAX_NUM, MAX_NUM < 10 ? " " : "");
    ...
    printf("|  *  .  = Empty cell                                       |\n");
    printf("|                                                           |\n");
    printf("|  Grid: %dx%d | Players: 3-5 | Turn Order: Round Robin    %s|\n", 
           GRID_SIZE, GRID_SIZE, GRID_SIZE < 10 ? " " : "");
    printf("+-----------------------------------------------------------+\n\n");
}

void print_horizontal_line(void) {
    printf("    +");
    for (int box = 0; box < GRID_SIZE / BOX_COLS; box++) {
...
void print_grid(void) {
    ...
    for (int c = 0; c < GRID_SIZE; c++) {
        ...
        if (c < 9) {
            ...
            printf("%2d", c + 1);  // For grid sizes > 9
        }
    }
    printf("\n");
    print_horizontal_line();
    ...
    for (int r = 0; r < GRID_SIZE; r++) {
        ...
        if (r < 9) {
            ...
            printf("%2d  |", r + 1);  // For grid sizes > 9
        }
        ...
        for (int c = 0; c < GRID_SIZE; c++) {
            ...
            if (cell->value == EMPTY_CELL) {
                ...
                if (cell->value < 10) {
                    ...
                    printf("%2d", cell->value);  // For grid sizes > 9
                }
            } else {
    ...
    printf("  Grid: %dx%d | Cells remaining: %d\n\n", GRID_SIZE, GRID_SIZE, local_cells_remaining);
}
        ...
        if (FD_ISSET(STDIN_FILENO, &read_fds)) {
            ...
            if (parse_place_command(input, &row, &col, &value)) {
                if (row < 0 || row >= GRID_SIZE || col < 0 || col >= GRID_SIZE) {
                    printf("[ERROR] Row and column must be 1-%d\n", GRID_SIZE);
                    continue;
                }
                if (value < 1 || value > MAX_NUM) {
                    printf("[ERROR] Number must be 1-%d\n", MAX_NUM);
                    continue;
                }
